package com.duoc.inkumanga.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleWebClientError(WebClientResponseException ex) {
        if (ex.getStatusCode().value() == 404) {
            ApiError error = new ApiError(404, "Manga no encontrado en Jikan API", ex.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
        }
        ApiError error = new ApiError(502, "Error al consultar Jikan API", ex.getMessage());
